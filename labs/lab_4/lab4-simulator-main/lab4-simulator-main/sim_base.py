...
def execute(instruction, oldPC):
    ...
    global M, R, rsp
    reserved = int(get_bits(instruction, 8, 7))
    icode = int(get_bits(instruction, 4, 7))
    a = int(get_bits(instruction, 2, 4))
    b = int(get_bits(instruction, 0, 2))

    if int(get_bits(instruction, 7, 8)) == 1:
        return oldPC
    if reserved == 1:
        if icode == 0:
            if b == 0:
